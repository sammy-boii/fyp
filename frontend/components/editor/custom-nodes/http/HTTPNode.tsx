...
export function HTTPNode({ data, id }: NodeProps<BaseNodeProps>) {
  ...
  // Get the HTTP node definition from registry
  const node = NODE_DEFINITIONS[NODE_TYPES.HTTP]
  const httpAction = node.actions[0]

  // Get method and URL for subtitle
  const method = data.config?.method || 'GET'
  const url = data.config?.url || ''

  // Method color mapping for badge
    ...
    <ContextMenu>
      <ContextMenuTrigger>
        <div className='relative group'>
            ...
            <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Are you sure you want to delete this HTTP node? This action
          ...
          <Card
            className={`relative w-52 p-4 rounded-lg border transition-all duration-300 bg-card ${
              data.isExecuting
              ...
              <div className='flex-1 min-w-0'>
                ...
                {url ? (
                  <div className='flex items-center gap-1.5'>
                    <p className='text-[10px] text-muted-foreground font-medium truncate max-w-20'>
...
export const HTTPNodeMemo = memo(HTTPNode)
